...
<edges> reported as DIRECTED, self-loops allowed, no multiple edges.
    ...
    with open(file_in, "r") as lines:
        ...
        if n_cols == 2:
            ...
            for key,vals in E.items():
                E[key][1] = [w/sum(vals[1]) for w in vals[1]]
        else:
...
def walk(i, Q_dist, idx_c):
    ...
    while JSD > epsilon:
        ...
        t += 1
        i = j
        # repeat the walk if it gets trapped
        if t > 10**5:
            print('limit hit %s %s %s' %(node, n, idx_c))
            # idx_c = np.where(Q_dist == sampled)[0][0]
            return walk(node, Q_dist, idx_c)
        # print(t, i, JSD)
    return t
